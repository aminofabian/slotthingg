...
export default function RechargeModal({ isOpen, onClose, game }: RechargeModalProps) {
                ...
                <div className="space-y-5">                  
                  ...
                  <div className="flex items-center justify-between border-b border-white/10 pb-3">
                    <Dialog.Title className="text-lg font-medium text-white flex items-center gap-2">
                      <div className="w-8 h-8 bg-[#6f42c1] rounded-md flex items-center justify-center">
                        <Plus className="w-4 h-4 text-white" />
                      </div>
                      <div className="flex flex-col">
                        <span className="text-base font-medium">Recharge</span>
                        <span className="text-xs text-white/60 font-normal">Add funds to <span className="text-white">{gameTitle || game.title}</span></span>
                      </div>
                    </Dialog.Title>
                    <button 
                      onClick={onClose}
                  ...
                  <form onSubmit={handleRecharge} className="space-y-5">
                    <div className="space-y-2">
                      <label htmlFor="recharge-amount" className="text-white/80 text-sm block">
                        Amount to Recharge
                      </label>
                      <div className="relative">
                        <input
                          ...
                          className="w-full p-2.5 pl-9 bg-black/20 text-white placeholder-white/30
                            rounded border border-white/10 focus:border-[#6f42c1]
                            focus:outline-none focus:ring-1 focus:ring-[#6f42c1] transition-colors"
                          required
                        />
                        <div className="absolute left-3 top-1/2 transform -translate-y-1/2 text-[#6f42c1]">
                      ...
                      {amount && parseFloat(amount) > 0 && (
                        <div className="mt-2 py-1.5 px-3 bg-[#6f42c1]/10 rounded border border-[#6f42c1]/20 flex items-center justify-between">
                          <span className="text-xs text-white/80">Total amount:</span>
                          <span className="font-medium text-sm text-[#6f42c1]">${parseFloat(amount).toFixed(2)}</span>
                        </div>
                      )}
                    </div>
                    ...
                    <button 
                      ...
                      className="w-full p-2.5 bg-[#6f42c1] hover:bg-[#6f42c1]/90
                        text-white rounded text-base font-medium
                        transition-colors flex items-center justify-center gap-2
                        focus:outline-none focus:ring-2 focus:ring-[#6f42c1] focus:ring-offset-1
