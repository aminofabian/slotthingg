                  ...
                  <>
                    {messages.map((msg) => (
                      <MotionDiv
                        ...
                        className={`flex ${msg.is_player_sender ? 'justify-end' : 'justify-start'} mb-3`}
                      >
                        {msg.type === 'message' ? (
                          <div
                            className={`max-w-[85%] sm:max-w-[75%] relative group ${
                              msg.is_player_sender
                                ? 'bg-gradient-to-br from-[#00ffff]/15 via-[#00ffff]/10 to-[#00ffff]/5 text-white shadow-md shadow-[#00ffff]/10 rounded-2xl rounded-br-sm border border-[#00ffff]/10'
                                : 'bg-gradient-to-br from-[#ff00ff]/15 via-[#ff00ff]/10 to-[#9400d3]/5 text-white shadow-md shadow-[#ff00ff]/10 rounded-2xl rounded-bl-sm border border-[#ff00ff]/10'
                            } px-5 py-3 backdrop-blur-sm
                            transition-all duration-200 hover:shadow-lg hover:-translate-y-0.5`}
                          >
                            {/* Sender name - only show for admin messages */}
                            {!msg.is_player_sender && (
                              <div className="text-xs font-medium tracking-wide text-[#ff00ff]/90 mb-1.5">
                                {msg.sender_name || 'Admin'}
                              </div>
                            )}
                            
                            <div className="text-sm sm:text-base break-words leading-relaxed font-light tracking-wide">
                              {msg.message}
                            </div>

                            {/* Time and status */}
                            <div className={`flex items-center gap-1.5 mt-1.5 text-[0.65rem] ${
                          ...
                          <div className="text-xs sm:text-sm text-white/40 py-2 px-3 italic tracking-wide font-light">
                            {msg.message}
                          </div>
                        )}
                      </MotionDiv>
